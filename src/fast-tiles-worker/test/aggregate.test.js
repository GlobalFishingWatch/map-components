...
test('Builds intermediate array buffer', () => {
  ...
  expect(arrayBuff[2]).toBe(17167)
  expect(arrayBuff[3]).toBe(17716)
  expect(arrayBuff[4]).toBe(0)
...
test('Big tile, big delta', () => {
  ...
  const arrayBuffers = rawTileToIntArrays(bigTileRaw, tileset)
  const agg = aggregate(arrayBuffers, {
    delta: 10,
    quantizeOffset,
...
test('Pick a value (delta 1)', () => {
  ...
  console.log(aggregated.features[0])
  console.log(aggregated.features[1])
  // // at 549, 1
